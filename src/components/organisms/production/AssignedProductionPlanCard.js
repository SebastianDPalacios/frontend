...
const AssignedProductionPlanCard = ({
  ...
  <Paper variant="outlined" sx={{ borderRadius: 2, p: 2 }}>
    ...
    >
      <Box>
        <Typography sx={{ fontWeight: 900 }}>{plan.baker_name || "Mi producción"}</Typography>
      ...
      <Chip
        size="small"
        label={plan.status === "completed" ? "Producción finalizada" : plan.status === "viewed" ? "Vista" : "Asignada"}
      ...
      {(Array.isArray(plan.items) ? plan.items : []).map((item) => (
        <Box key={item.id} sx={{ p: 1.5, borderRadius: 2, bgcolor: "background.default" }}>
          ...
          >
            ...
            {item.started_at || item.production_batch_id ? (
              <AppButton
                variant="outlined"
                color="secondary"

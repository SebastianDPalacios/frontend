...
const DashboardView = ({ stats = {}, insights = {}, currentUser = null }) => {
  const bakerOnly = isBakerOnlyUser(currentUser);
  const packagingOnly = isPackagingOnlyUser(currentUser);
  const focusedProductionUser = bakerOnly || packagingOnly;

  const canManageUsers = !focusedProductionUser && hasPermission(currentUser, "users.manage");
  const canManageCustomers = !focusedProductionUser && hasPermission(currentUser, "customers.manage");
  const canManageProducts = !focusedProductionUser && hasPermission(currentUser, "products.manage");
  const canManageOrders = !focusedProductionUser && hasPermission(currentUser, "orders.manage");
  const canManageProduction = !focusedProductionUser && hasPermission(currentUser, "production.manage");
  const canReportProduction = !packagingOnly && (bakerOnly || hasPermission(currentUser, "production.baker"));
  const canCountPackaging = !bakerOnly && (packagingOnly || hasPermission(currentUser, "production.packaging"));
  const canManageInventory = !focusedProductionUser && hasPermission(currentUser, "inventory.manage");
  const canViewReports = !focusedProductionUser && hasPermission(currentUser, "reports.view");
  ...
  const signals = [
    ...
    canManageOrders && insights.orders
      ? {
          ...
          helper: "Pedidos despachados que todavia no generan comision ni aparecen en la liquidacion.",
          href: "/orders/history",
      ...
      : null,
    (canManageProduction || canCountPackaging) && insights.production
      ? {
  ...
  const monthly = canViewReports ? insights.monthly : null;
  return (
    <>
      <DashboardWelcomePanel
        ...
        canReportProduction={canReportProduction}
        canCountPackaging={canCountPackaging}
      ...
      <Grid container spacing={2} sx={{ mb: 2.5 }}>
        {canManageOrders && insights.orders ? (
          <>
            ...
            <Grid item xs={12} sm={6} md={4} lg={2.4}>
              <DashboardMetricCard title="Entregados" value={insights.orders.delivered} helper="Con comision generada" icon={<CheckCircleIcon />} color="success" />
            </Grid>
          </>
        ) : null}
        {canManageUsers ? (
        ...
        {canManageProducts ? (
          <Grid item xs={12} sm={6} md={4} lg={2.4}>
            <DashboardMetricCard title="Productos" value={stats.products} helper="Catalogo de venta" icon={<Inventory2Icon />} color="success" />
          </Grid>
        ) : null}
      </Grid>
